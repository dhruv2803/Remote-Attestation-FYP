...
nodes = [100,200,500,700, 1000, 5000, 10000]
  ...
  for j in range(0, simRepetitions):
    ...
    while time.time() < (simulationStart + simulationDuration):
      ...
      if ((currentSecond - lastSecond) < 1) and (secondIterations < trustQueryRate):
        ...
        pathFound,last = findPath(G,node, prover,evidenceList,timeFunction,minReliability)
        if pathFound:
          trustQueryHits += 1
        else:
          trustQueryMisses += 1
    ...
    hitPercentages.append((trustQueryHits / (trustQueryHits + trustQueryMisses)) * 100)
